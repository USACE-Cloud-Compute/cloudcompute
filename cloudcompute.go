...
const (
	computeTag  string = "compute"
	eventTag    string = "event"
	payloadTag  string = "payload"
	manifestTag string = "manifest"
	jobTag      string = "job"
)
	...
	for {
		...
		cr.Run(func(event Event) {
			...
			if cc.EventProcessor != nil {
				...
				event, err = cc.EventProcessor.Process(event)
				if err != nil {
					log.Printf("error postprocessing the event. identifier %s, error %s\n", eventIdentifier, err)
					return
				}
			}
			///////////////////////////////
			...
			for i, manifest := range event.Manifests {
				...
				//add tags:
				manifest.Tags[computeTag] = cc.ID.String()
				manifest.Tags[eventTag] = event.ID.String()
				manifest.Tags[manifestTag] = manifest.ManifestID.String()
				manifest.Tags[jobTag] = jobID.String()
...
func (cm *ComputeManifest) WritePayload(storeProfile string) error {
	if cm.payloadID == uuid.Nil {
		payloadId := uuid.New()
		ccStoreConfig := CcStoreInput{
			ManifestId:     cm.ManifestID.String(),
			PayloadId:      payloadId.String(),
			CcStoreProfile: storeProfile,
		}
		computeStore, err := NewCcStore(&ccStoreConfig)
		if err != nil {
			return err
		}
		p := Payload{
